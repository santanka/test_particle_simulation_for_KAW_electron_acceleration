...
# initial energy
#initial_energy_perp_eq_array_eV = np.array([1E0])    #[eV]
initial_energy_perp_eq_array_eV = np.array([1E0, 1E1, 1E2, 2E2, 3E2, 4E2, 5E2, 6E2, 7E2, 8E2, 9E2, 1E3])    #[eV]
initial_energy_perp_eq_array = initial_energy_perp_eq_array_eV * elementary_charge    #[J]

initial_mu_array = initial_energy_perp_eq_array / magnetic_flux_density(0E0)    #[J/T]

initial_psi = - np.pi / 2E0    #[rad]
end_psi = - np.pi    #[rad]
...
def function_0(mu, mlat_rad, theta_per_2_trapping_freq):
    return (delta(mlat_rad) + epsilon(mlat_rad)) * (np.sqrt(2E0 * energy_wave_potential(mlat_rad)) * theta_per_2_trapping_freq + np.sqrt(energy_wave_phase_speed(mlat_rad)))**2E0 + delta(mlat_rad) * magnetic_flux_density(mlat_rad) * mu - energy_wave_potential(mlat_rad)

def Newton_method_function_0(mu, theta_per_2_trapping_freq):
    initial_mlat_rad = np.pi / 2E0
    mlat_rad_before_update = initial_mlat_rad
    count_iteration = 0
    while True:
        diff = function_0(mu, mlat_rad_before_update, theta_per_2_trapping_freq) / gradient_mlat(function_0, mlat_rad_before_update, mu, theta_per_2_trapping_freq)
        if abs(diff) > 1E-1:
            diff = np.sign(diff)
...
def K_perp(mlat_rad, mu):
    return mu * magnetic_flux_density(mlat_rad)    #[J]

def K_total(mlat_rad, mu, theta_per_2_trapping_freq):
    return K_para(mlat_rad, theta_per_2_trapping_freq) + K_perp(mlat_rad, mu)    #[J]

def S_value_theta(mlat_rad, mu, theta_per_2_trapping_freq):
    return ((delta(mlat_rad) + epsilon(mlat_rad)) * K_para(mlat_rad, theta_per_2_trapping_freq) + delta(mlat_rad) * K_perp(mlat_rad, mu)) / energy_wave_potential(mlat_rad)    #[]
...
def make_data(args):
    count_mu, count_i, initial_mlat_rad_list, initial_theta_per_2_trappint_freq_list, initial_theta_list, initial_psi = args

    initial_mu = initial_mu_array[count_mu]
    initial_mlat_rad = initial_mlat_rad_list[count_i]
    initial_theta_per_2_trapping_freq = initial_theta_per_2_trappint_freq_list[count_i]
    initial_theta = initial_theta_list[count_i]

    mlat_rad_array_RK4 = np.array([initial_mlat_rad])
    theta_array_RK4 = np.array([initial_theta])
    psi_array_RK4 = np.array([initial_psi])
    time_array_RK4 = np.array([0E0])

    mlat_old = initial_mlat_rad
    ...
    pitch_angle_array_RK4 = np.zeros(len(mlat_rad_array_RK4))
    S_array_RK4 = np.zeros(len(mlat_rad_array_RK4))
    trapping_frequency_array_RK4 = np.zeros(len(mlat_rad_array_RK4))
    for count_i in range(len(mlat_rad_array_RK4)):
        energy_array_RK4[count_i], pitch_angle_array_RK4[count_i] = kinetic_energy_and_pitch_angle_rad(initial_mu, mlat_rad_array_RK4[count_i], theta_array_RK4[count_i])
        S_array_RK4[count_i] = S_value(energy_array_RK4[count_i], mlat_rad_array_RK4[count_i], pitch_angle_array_RK4[count_i])
        trapping_frequency_array_RK4[count_i] = trapping_frequency(mlat_rad_array_RK4[count_i])
    
    return mlat_deg_array_RK4, energy_array_RK4, psi_array_RK4, time_array_RK4, pitch_angle_array_RK4, initial_mu, theta_array_RK4, S_array_RK4, trapping_frequency_array_RK4



def main(count_mu):
    initial_mu = initial_mu_array[count_mu]
    initial_K_perp = initial_mu * magnetic_flux_density(0E0)
    ...
    for count_i in range(trajectory_number):
        ...
        initial_mlat = Newton_method_function_0(initial_mu, theta_per_2_trapping_freq_initial)
        Kpara_init = K_para(initial_mlat, theta_per_2_trapping_freq_initial)
        if Kpara_init >= energy_wave_phase_speed(initial_mlat) and theta_per_2_trapping_freq_initial >= - np.sqrt(energy_wave_phase_speed(initial_mlat) / (2E0 * energy_wave_potential(initial_mlat))) and theta_per_2_trapping_freq_initial <= np.sqrt(energy_wave_phase_speed(initial_mlat) / (2E0 * energy_wave_potential(initial_mlat))):
            initial_mlat_rad_list.append(initial_mlat)
            initial_pitch_angle_rad = np.arctan(np.sqrt(K_perp(initial_mlat, initial_mu) / K_para(initial_mlat, theta_per_2_trapping_freq_initial)))
    ...
    ax_2_1 = fig_2.add_subplot(411, xlabel=r'$\psi$ [$\pi$ $\mathrm{rad}$]', ylabel=r'$K$ [$\mathrm{eV}$]', yscale='log', xlim=(end_psi/np.pi, 0))
    ax_2_2 = fig_2.add_subplot(412, xlabel=r'$\psi$ [$\pi$ $\mathrm{rad}$]', ylabel=r'$K_{\perp}$ [$\mathrm{eV}$]', yscale='log', xlim=(end_psi/np.pi, 0))
    ax_2_3 = fig_2.add_subplot(413, xlabel=r'$\psi$ [$\pi$ $\mathrm{rad}$]', ylabel=r'$K_{\parallel}$ [$\mathrm{eV}$]', yscale='log', xlim=(end_psi/np.pi, 0))
    ...
    ax_3_2 = fig_3.add_subplot(412, xlabel=r'$\psi$ [$\pi$ $\mathrm{rad}$]', ylabel=r'$\mathrm{S}$', xlim=(end_psi/np.pi, 0), yscale='log')
    ...
    if __name__ == '__main__':
        ...
        args = []
        for count_i in range(len(initial_mlat_rad_list)):
            args.append([count_mu, count_i, initial_mlat_rad_list, theta_per_2_trapping_freq_initial_list, initial_theta_list, initial_psi])
        
        with Pool(num_process) as p:
            results = p.map(make_data, args)

        for result in results:
            mlat_deg_array_RK4, energy_array_RK4, psi_array_RK4, time_array_RK4, pitch_angle_array_RK4, initial_mu, theta_array_RK4, S_array_RK4, trapping_frequency_array_RK4 = result

            theta_per_2_trapping_freq_color = theta_array_RK4[0] / 2E0 / trapping_frequency_array_RK4[0] * np.ones(len(mlat_deg_array_RK4))
            ...
            ax_2_2.scatter(psi_array_RK4 / np.pi, energy_array_RK4 * np.sin(pitch_angle_array_RK4)**2E0 / elementary_charge, c=theta_per_2_trapping_freq_color, alpha=0.3, cmap=cmap_color, vmin=theta_per_2_trapping_freq_array_vmin, vmax=theta_per_2_trapping_freq_array_vmax, s=1)
            ax_2_2.scatter(psi_array_RK4[0] / np.pi, energy_array_RK4[0] * np.sin(pitch_angle_array_RK4[0])**2E0 / elementary_charge, c='black', s=50)
            ax_2_2.scatter(psi_array_RK4[-1] / np.pi, energy_array_RK4[-1] * np.sin(pitch_angle_array_RK4[-1])**2E0 / elementary_charge, c='black', s=50)
            ax_2_3.scatter(psi_array_RK4 / np.pi, energy_array_RK4 * np.cos(pitch_angle_array_RK4)**2E0 / elementary_charge, c=theta_per_2_trapping_freq_color, alpha=0.3, cmap=cmap_color, vmin=theta_per_2_trapping_freq_array_vmin, vmax=theta_per_2_trapping_freq_array_vmax, s=1)
            ax_2_3.scatter(psi_array_RK4[0] / np.pi, energy_array_RK4[0] * np.cos(pitch_angle_array_RK4[0])**2E0 / elementary_charge, c='black', s=50)
            ax_2_3.scatter(psi_array_RK4[-1] / np.pi, energy_array_RK4[-1] * np.cos(pitch_angle_array_RK4[-1])**2E0 / elementary_charge, c='black', s=50)
            ...
            ax_3_1.scatter(psi_array_RK4 / np.pi, trapping_frequency_array_RK4 / np.pi, c=theta_per_2_trapping_freq_color, alpha=0.3, cmap=cmap_color, vmin=theta_per_2_trapping_freq_array_vmin, vmax=theta_per_2_trapping_freq_array_vmax, s=1)
            ...
            ax_3_2.scatter(psi_array_RK4 / np.pi, S_array_RK4, c=theta_per_2_trapping_freq_color, alpha=0.3, cmap=cmap_color, vmin=theta_per_2_trapping_freq_array_vmin, vmax=theta_per_2_trapping_freq_array_vmax, s=1)
            ax_3_2.scatter(psi_array_RK4[0] / np.pi, S_array_RK4[0], c='black', s=50)
            ax_3_2.scatter(psi_array_RK4[-1] / np.pi, S_array_RK4[-1], c='black', s=50)
            ax_3_3.scatter(psi_array_RK4 / np.pi, theta_array_RK4 / 2E0 / trapping_frequency_array_RK4, c=theta_per_2_trapping_freq_color, alpha=0.3, cmap=cmap_color, vmin=theta_per_2_trapping_freq_array_vmin, vmax=theta_per_2_trapping_freq_array_vmax, s=1)
            ax_3_3.scatter(psi_array_RK4[0] / np.pi, theta_array_RK4[0] / 2E0 / trapping_frequency_array_RK4[0], c='black', s=50)
            ax_3_3.scatter(psi_array_RK4[-1] / np.pi, theta_array_RK4[-1] / 2E0 / trapping_frequency_array_RK4[-1], c='black', s=50)
    ...
    initial_K_perp = round(initial_K_perp / elementary_charge)
    
    fig_1.suptitle(r'$K_{\perp}(\lambda \, = \, 0) \, = \,$' + str(int(initial_K_perp)) + r' [$\mathrm{eV}$]')
    fig_2.suptitle(r'$K_{\perp}(\lambda \, = \, 0) \, = \,$' + str(int(initial_K_perp)) + r' [$\mathrm{eV}$]')
    fig_3.suptitle(r'$K_{\perp}(\lambda \, = \, 0) \, = \,$' + str(int(initial_K_perp)) + r' [$\mathrm{eV}$]')
    ...
    energy_Kperp_eV = np.zeros(len(mlat_deg_array))
    for count_i in range(len(mlat_deg_array)):
        ...
        energy_Kperp_eV[count_i] = K_perp(mlat_rad_array[count_i], initial_mu) / elementary_charge
    ...
    ax_1.plot(mlat_deg_array, energy_Kperp_eV, color='orange', linewidth=4, label=r'$K_{\perp}$', alpha=0.6)
    ...
    sm.set_array([])
    cbar = fig_1.colorbar(sm, ax=ax_1, label=r'$(\theta / 2 \omega_{\mathrm{t}})_{\mathrm{i}}$')

    fig_1.tight_layout()

    if not os.path.exists(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}'):
        os.makedirs(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}')

    fig_1.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_enlarged.png', dpi=100)
    fig_1.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_enlarged.pdf', dpi=100)

    ax_1.set_xlim(0E0, mlat_upper_limit_deg)
    ax_1.set_ylim(1E1, 1E5)
    fig_1.tight_layout()

    fig_1.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9.png', dpi=100)
    fig_1.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9.pdf', dpi=100)
    ...
    fig_2.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_psi_vs_energy.png', dpi=100)
    fig_2.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_psi_vs_energy.pdf', dpi=100)
    ...
    fig_3.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_psi_vs_S_omegat.png', dpi=100)
    fig_3.savefig(f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/energy_trajectory_S_not_constant_initialmu_constant/end_psi_{(end_psi/np.pi):.2f}/energy_trajectory_S_not_constant_initialmu_constant_{str(int(initial_K_perp))}_Earth_L_9_psi_vs_S_omegat.pdf', dpi=100)

for count_mu in range(len(initial_mu_array)):
    main(count_mu)