...
def main(args):
    ...
    dir_name = f'/mnt/j/KAW_simulation_data/single_test_particle/keisan/phase_trapping_region2/test_particle_simulation_phase_trapping_initialpsi_{initial_psi_main/np.pi:.2f}_pi'
    ...
    function_phase_trapping_time_derivative_array = np.zeros_like(mlat_rad_array)
    function_phase_trapping_time_derivative_2_array = np.zeros_like(mlat_rad_array)
    ...
    for count_i in range(len(mlat_rad_array)):
        function_phase_trapping_array[count_i], function_phase_trapping_time_derivative_array[count_i], function_saddle_point_array[count_i], function_resonant_scattering_array[count_i], region_array[count_i] = region_detection(initial_mu_main, theta_array[count_i], psi_array[count_i], mlat_rad_array[count_i], time_array[count_i], time_array[-1])

    for count_i in range(len(mlat_rad_array)):
        if count_i == 0 or count_i == len(mlat_rad_array) - 1:
            function_phase_trapping_time_derivative_2_array[count_i] = np.nan
        else:
            function_phase_trapping_time_derivative_2_array[count_i] = (function_phase_trapping_time_derivative_array[count_i + 1] - function_phase_trapping_time_derivative_array[count_i - 1]) / (time_array[count_i + 1] - time_array[count_i - 1])
    ...
    ax_2_3 = fig.add_subplot(gs[2, 1], xlabel=r'$\psi$ [$\pi$ rad]', ylabel=r'Equilibrium points [$\pi$ rad]', xlim=[-1, 1], ylim=[-1.01, 0.01])
    ax_3_1 = fig.add_subplot(gs[0, 2], xlabel=r'$\psi$ [$\pi$ rad]', ylabel=r'vs. orange line', xlim=[-1, 1])
    ax_3_2 = fig.add_subplot(gs[1, 2], xlabel=r'$\psi$ [$\pi$ rad]', ylabel=r'vs. orange line (time derivative) / $\omega_{\mathrm{t}}$', xlim=[-1, 1], ylim=[-5, 5])
    ax_3_3 = fig.add_subplot(gs[2, 2], xlabel=r'$\psi$ [$\pi$ rad]', ylabel=r'vs. orange line (second derivative of time) / $\omega_{\mathrm{t}}^{2}$', xlim=[-1, 1], ylim=[-5, 5])
    ...
    ax_2_3.scatter(psi_array / np.pi, (psi_array - function_saddle_point_array)/np.pi, c=color_target, cmap=cmap_color, s=1E0, vmin=vmin, vmax=vmax)
    ax_2_3.scatter(psi_array / np.pi, -(psi_array - function_saddle_point_array + np.pi)/np.pi, c=color_target, cmap=cmap_color, s=1E0, vmin=vmin, vmax=vmax)
    ax_2_3.scatter(psi_array[0] / np.pi, (psi_array[0] - function_saddle_point_array[0])/np.pi, c='lightgrey', s=200, marker='o', edgecolors='k', zorder=1)
    ax_2_3.scatter(psi_array[-1] / np.pi, (psi_array[-1] - function_saddle_point_array[-1])/np.pi, c='orange', s=200, marker='D', edgecolors='k', zorder=1)
    ax_2_3.scatter(psi_array[0] / np.pi, -(psi_array[0] - function_saddle_point_array[0] + np.pi)/np.pi, c='lightgrey', s=200, marker='o', edgecolors='k', zorder=1)
    ax_2_3.scatter(psi_array[-1] / np.pi, -(psi_array[-1] - function_saddle_point_array[-1] + np.pi)/np.pi, c='orange', s=200, marker='D', edgecolors='k', zorder=1)
    ax_2_3.plot(np.linspace(-1, 0, 100), np.linspace(-1, 0, 100), c='k', linewidth=4E0, alpha=0.5)
    ...
    ax_3_2.axhline(y=np.pi/2E0, c='k', linewidth=4E0, alpha=0.5)

    ax_3_3.scatter(psi_array / np.pi, function_phase_trapping_time_derivative_2_array / trapping_frequency_array**2E0, c=color_target, cmap=cmap_color, s=1E0, vmin=vmin, vmax=vmax)
    ax_3_3.axhline(y=0E0, c='k', linewidth=4E0, alpha=0.5)
    ax_3_3.axhline(y=(np.pi/2E0)**2E0, c='k', linewidth=4E0, alpha=0.5)
