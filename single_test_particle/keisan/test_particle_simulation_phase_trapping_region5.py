...
def main(args):
    ...
    # Equilibrium points
    stable_equilibrium_point_array = np.zeros_like(mlat_rad_array)
    unstable_equilibrium_point_array = np.zeros_like(mlat_rad_array)
    point_theta_0_another_unstable_equilibrium_point_array = np.zeros_like(mlat_rad_array)

    for count_i in range(len(mlat_rad_array)):
        if S_value_array[count_i] <= 1E0 and S_value_array[count_i] >= 0E0:
            stable_equilibrium_point_array[count_i] = - np.arcsin(S_value_array[count_i])
            unstable_equilibrium_point_array[count_i] = - np.pi + np.arcsin(S_value_array[count_i])
            point_theta_0_another_unstable_equilibrium_point_array[count_i] = iteration_another_point(S_value_array[count_i])
        else:
            stable_equilibrium_point_array[count_i] = np.nan
            unstable_equilibrium_point_array[count_i] = np.nan
            point_theta_0_another_unstable_equilibrium_point_array[count_i] = np.nan
    ...
    ax_2_2.axhline(y=1.2E0, c='k', linewidth=4E0, alpha=0.5)
    ...
    ylim_enlarged_1_3 = [np.nanmin(theta_array / 2E0 / trapping_frequency_array)-0.1, np.nanmax(theta_array / 2E0 / trapping_frequency_array)+0.1]
    if ylim_enlarged_1_3[0] < -3E0:
        ylim_enlarged_1_3[0] = -3E0
    if ylim_enlarged_1_3[1] > 3E0:
    ...
    if ylim_enlarged_3_3[1] > 3E0:
        ylim_enlarged_3_3[1] = 3E0
    ax_3_3.set_ylim(tuple(ylim_enlarged_3_3))
