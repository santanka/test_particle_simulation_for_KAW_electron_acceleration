...
#21:wave phase & wave trapping frequency & delta_m & stable/unstable point time variation
#22:psi-theta plot
...
if (channel == 11):
    ...
    dp_wavefreq = 2E0 * np.pi * 0.15 #[rad/s]
...
if (channel == 16):
    ...
    b0 = B0_eq / np.cos(dp_mlat_rad)**6E0 * np.sqrt(1E0 + 3E0 * np.sin(dp_mlat_rad)**2E0) * 1E4     #[G]
    dp_kpara = np.sqrt(2E0 * np.pi * number_density_ion * mass_ion * pressure_ion) / b0**2E0 * np.sqrt(4E0 * np.pi + b0**2E0 / (pressure_ion + pressure_electron)) * np.sign(dp_mlat_rad)   #[rad/cm]
    dp_wavefreq = 2E0 * np.pi * 0.15E0 #[rad/s]
    kperp_rhoi = 2E0 * np.pi
    dp_Alfven_speed = b0 / np.sqrt(4E0 * np.pi * mass_ion * number_density_ion)  #[cm/s]
    dp_beta_ion = pressure_ion / (b0**2E0 / 2E0 / 4E0 / np.pi)    #[]
    dp_kpara = dp_wavefreq / kperp_rhoi / dp_Alfven_speed * (dp_beta_ion + 2E0 * temperature_ion / (temperature_ion + temperature_electron)) * np.sign(dp_mlat_rad)     #[rad/cm]
    dp_phasespeed = dp_wavefreq / dp_kpara / 1E2    #[m s-1]
...
if (channel == 22 and switch_delta_Epara == 1E0):
    ...
    dp_S = dp_kpara / dp_omega_tr**2E0 * (dp_mu/mass_electron + (1E0 + dp_beta_ion * (temperature_ion + temperature_electron) / (dp_beta_ion * (temperature_ion + temperature_electron) + 2E0 * temperature_ion)) * (dp_v_para*1E2)**2E0 / dp_B0) * dp_dB0_dz    #[]

    if (switch_delta_Bpara == 1):
        dp_Gamma_tr = np.sqrt((1E0 - dp_mu / (elementary_charge/1E1*speed_of_light*1E2) / dp_Phi * dp_Phi_B / dp_B0)**2E0 + 1E0 / dp_kpara**2E0 * (dp_mu / (elementary_charge/1E1*speed_of_light*1E2) / dp_Phi * dp_d_Phi_B_B0_dz)**2E0)    #[]
    ...
    fig = plt.figure(figsize=(20, 20), dpi=100, tight_layout=True)
    ax = fig.add_subplot(211, xlabel=r'wave phase $\psi$ [$\times \pi$ rad]', ylabel=r'$\frac{\theta}{2\omega_{\mathrm{t}}}$')
    ...
    ax1 = fig.add_subplot(212, xlabel=r'time [s]')
    ax1.plot(dp_time, dp_omega_tr, lw=4, color=r'b', label=r'$\omega_{\mathrm{tr}}$ [Hz]')
    ax1.plot(dp_time, dp_Gamma_tr, lw=4, color=r'r', label=r'$\Gamma_{\mathrm{tr}}$')
    ax1.plot(dp_time, dp_S, lw=4, color=r'g', label=r'$S$')
    ax1.minorticks_on()
    ax1.grid(which="both", alpha=0.3)
