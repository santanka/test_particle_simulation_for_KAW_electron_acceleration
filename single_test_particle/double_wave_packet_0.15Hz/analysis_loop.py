...
initial_wavephase_list       = [0E0]       #[deg]
gradient_parameter_list      = [2E0]       #[]
wave_threshold          = 5E0       #[deg]

wavekind_list                = [r'EparaBpara', r'Epara']
switch_delta_Epara_list      = [1E0, 1E0]
switch_delta_Eperp_perp = 0E0
switch_delta_Eperp_phi  = 0E0
switch_delta_Bpara_list      = [1E0, 0E0]
...
def main(count_grad, count_angle, count_kind, particle_file_number, channel):
    ...
    dir_name = f'/mnt/j/KAW_simulation_data/single_test_particle/double_wave_packet/0.15Hz/results_particle_{str(int(wave_scalar_potential))}V' \
    ...
    if (channel == 1):
        ...
        if (check_file_exists(f'{dir_name}/result_parallel_force_times_vpara_scalar_potential/particle_trajectory{particle_file_number}.png') == True):
            return
    elif (channel == 16):
        if (check_file_exists(f'{dir_name}/result_pendulum_theory_time_variation/particle_trajectory{particle_file_number}.png') == True):
            return
    elif (channel == 22):
        if (check_file_exists(f'{dir_name}/result_psi-theta_plot/particle_trajectory{particle_file_number}.png') == True):
            return
    elif (channel == 23):
        if (check_file_exists(f'{dir_name}/result_pendulum_theory_time_variation_using_mu/particle_trajectory{particle_file_number}.png') == True):
            return
    elif (channel == 24):
    ...
    if (channel == 14):
        ...
        ylim = ax.get_ylim()
        ax.hlines(mass_electron/mass_ion, dp_time[0], dp_time[-1], color='dimgrey', lw=4, linestyles='dashed', alpha=0.5)
        ax.set_ylim(ylim)
        ax.minorticks_on()
        ax.grid(which="both", alpha=0.3)
    ...
    if ((channel == 22 or channel == 23 or channel == 24) and switch_delta_Epara == 1E0):
        ...
        if (channel == 24 and switch_delta_Bpara == 0E0):
            return
        ...
        dp_omega_tr = wavecheck(array_size, dp_omega_tr, dp_wavephase)    #[rad/s]
        ...
        if (switch_delta_Bpara == 1):
            ...
            dp_Gamma_tr = wavecheck(array_size, dp_Gamma_tr, dp_wavephase)    #[]
        ...
        if (channel == 22):
            ...
            mkdir(f'{dir_name}/result_psi-theta_plot')
            fig.savefig(f'{dir_name}/result_psi-theta_plot/particle_trajectory{particle_file_number}.png')
        ...
        if (channel == 23):
            ...
            if (switch_delta_Bpara == 1E0):
                ...
                dp_delta_m = wavecheck(array_size, dp_delta_m, dp_wavephase)    #[rad]
            ...
            mkdir(f'{dir_name}/result_pendulum_theory_time_variation_using_mu')
            fig.savefig(f'{dir_name}/result_pendulum_theory_time_variation_using_mu/particle_trajectory{particle_file_number}.png')
