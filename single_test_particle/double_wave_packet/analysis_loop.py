...
#16:wave phase & wave trapping frequency & delta_m & stable/unstable point time variation
...
def main(count_grad, count_angle, count_kind, particle_file_number, channel):
    ...
    if (channel == 16):
        ...
        dp_ion_Larmor_radius = (speed_of_light*1E2) * np.sqrt(2E0*mass_ion*pressure_ion/number_density_ion) / (elementary_charge/1E1*speed_of_light*1E2) / dp_B0   #[cm]

        dp_kperp_1 = 2E0 * np.pi / dp_ion_Larmor_radius * np.ones(array_size) #[rad/cm]
        dp_kperp_2 = 2E0 * np.pi / dp_ion_Larmor_radius * np.ones(array_size) #[rad/cm]
