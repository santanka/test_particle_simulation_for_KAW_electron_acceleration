...
initial_wavephase_list       = [90E0, 180E0, 270E0, 90E0, 180E0, 270E0]       #[deg]
...
#
    ...
    if (channel == 12):
        ...
        dp_deltaBpara_1 = Alpha * g_function_1 / dp_B0 * np.cos(dp_wavephase_1) * switch_delta_Bpara
        dp_deltaBpara_2 = Alpha * g_function_2 / dp_B0 * np.cos(dp_wavephase_2) * switch_delta_Bpara
        dp_deltaBpara_sum = dp_deltaBpara_1 + dp_deltaBpara_2

        dp_deltaEpara_1 = (2E0 + pressure_electron / pressure_ion) * dp_kpara_1 * (wave_scalar_potential*1E8/(speed_of_light*1E2)) * g_function_1 * np.sin(dp_wavephase_1) * switch_delta_Epara
        dp_deltaEpara_2 = (2E0 + pressure_electron / pressure_ion) * dp_kpara_2 * (wave_scalar_potential*1E8/(speed_of_light*1E2)) * g_function_2 * np.sin(dp_wavephase_2) * switch_delta_Epara
        ...
        B0_function_1 = 2E0 * Alpha * (- 1E0 / dp_B0**2E0 * dp_dB0_dz * g_function_1) * (dp_Delta_real_1 * np.cos(dp_wavephase_1) - dp_Delta_imag_1 * np.sin(dp_wavephase_1)) * switch_delta_Bpara
        B0_function_2 = 2E0 * Alpha * (- 1E0 / dp_B0**2E0 * dp_dB0_dz * g_function_2) * (dp_Delta_real_2 * np.cos(dp_wavephase_2) - dp_Delta_imag_2 * np.sin(dp_wavephase_2)) * switch_delta_Bpara
        B0_function_sum = B0_function_1 + B0_function_2

        kpara_function_1 = 2E0 * Alpha * (- dp_kpara_1 * g_function_1 / dp_B0) * (dp_Delta_real_1 * np.sin(dp_wavephase_1) - dp_Delta_imag_1 * np.cos(dp_wavephase_1)) * switch_delta_Bpara
        kpara_function_2 = 2E0 * Alpha * (- dp_kpara_2 * g_function_2 / dp_B0) * (dp_Delta_real_2 * np.sin(dp_wavephase_2) - dp_Delta_imag_2 * np.cos(dp_wavephase_2)) * switch_delta_Bpara
        kpara_function_sum = kpara_function_1 + kpara_function_2

        dg_dz_function_1 = 2E0 * Alpha * (dg_dz_1 / dp_B0) * (dp_Delta_real_1 * np.cos(dp_wavephase_1) - dp_Delta_imag_1 * np.sin(dp_wavephase_1)) * switch_delta_Bpara
        dg_dz_function_2 = 2E0 * Alpha * (dg_dz_2 / dp_B0) * (dp_Delta_real_2 * np.cos(dp_wavephase_2) - dp_Delta_imag_2 * np.sin(dp_wavephase_2)) * switch_delta_Bpara
...
if (__name__ == '__main__'):
    ...
    with Pool(processes=num_processes) as pool:
        pool.map(main_loop, [(count_angle, count_kind, count_i, channel_idx) 
                            for count_angle in range(6)
                            for count_kind in range(3)
